...
class PreTrainedModel(nn.Module, ModuleUtilsMixin):
    r""" Base class for all models.
        ...
        :class:`~transformersdiffpruning.PreTrainedModel` takes care of storing the configuration of the models and handles methods for loading/downloading/saving models
        as well as a few methods common to all models to (i) resize the input embeddings and (ii) prune heads in the self-attention heads.

        Class attributes (overridden by derived classes):
            - ``config_class``: a class derived from :class:`~transformersdiffpruning.PretrainedConfig` to use as configuration class for this model architecture.
            - ``pretrained_model_archive_map``: a python ``dict`` of with `short-cut-names` (string) as keys and `url` (string) of associated pretrained weights as values.
            - ``load_tf_weights``: a python ``method`` for loading a TensorFlow checkpoint in a PyTorch model, taking as arguments:

                - ``model``: an instance of the relevant subclass of :class:`~transformersdiffpruning.PreTrainedModel`,
                - ``config``: an instance of the relevant subclass of :class:`~transformersdiffpruning.PretrainedConfig`,
    ...
    def save_pretrained(self, save_directory):
        """ Save a model and its configuration file to a directory, so that it
            can be re-loaded using the `:func:`~transformersdiffpruning.PreTrainedModel.from_pretrained`` class method.
        """
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, *model_args, **kwargs):
        ...
        Parameters:
            ...
            config: (`optional`) one of:
                - an instance of a class derived from :class:`~transformersdiffpruning.PretrainedConfig`, or
                - a string valid as input to :func:`~transformersdiffpruning.PretrainedConfig.from_pretrained()`
...
class SQuADHead(nn.Module):
    ...
    Parameters:
        config (:class:`~transformersdiffpruning.XLNetConfig`): Model configuration class with all the parameters of the model.
