...
class PreTrainedEncoderDecoder(nn.Module):
    r"""
        :class:`~transformersdiffpruning.PreTrainedEncoderDecoder` is a generic model class that will be
        ...
        **kwargs
        ...
        Params:
            encoder_pretrained_model_name_or_path: information necessary to initiate the encoder. Either:
                ...
                - a path to a `directory` containing model weights saved using :func:`~transformersdiffpruning.PreTrainedModel.save_pretrained`, e.g.: ``./my_model_directory/encoder``.
            ...
            decoder_pretrained_model_name_or_path: information necessary to initiate the decoder. Either:
                ...
                - a string with the `shortcut name` of a pre-trained model to load from cache or download, e.g.: ``bert-base-uncased``.
                - a string with the `identifier name` of a pre-trained model that was user-uploaded to our S3, e.g.: ``dbmdz/bert-base-german-cased``.
                - a path to a `directory` containing model weights saved using :func:`~transformersdiffpruning.PreTrainedModel.save_pretrained`, e.g.: ``./my_model_directory/decoder``.
                - a path or url to a `tensorflow index checkpoint file` (e.g. `./tf_model/model.ckpt.index`). In this case, ``from_tf`` should be set to True and a configuration object should be provided as ``config`` argument. This loading path is slower than converting the TensorFlow checkpoint in a PyTorch model using the provided conversion scripts and loading the PyTorch model afterwards.
    ...
    def save_pretrained(self, save_directory):
        ...
        that it can be loaded using `:func:`~transformersdiffpruning.PreTrainedEncoderDecoder.from_pretrained`
...
class Model2Model(PreTrainedEncoderDecoder):
    r"""
        :class:`~transformersdiffpruning.Model2Model` instantiates a Seq2Seq2 model
