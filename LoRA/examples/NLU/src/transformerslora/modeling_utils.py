...
class ModuleUtilsMixin:
    ...
    def reset_memory_hooks_state(self):
        ...
        :func:`~transformerslora.modeling_utils.ModuleUtilsMixin.add_memory_hooks`).
        """
        for module in self.modules():
...
class PreTrainedModel(nn.Module, ModuleUtilsMixin, GenerationMixin):
    ...
    :class:`~transformerslora.PreTrainedModel` takes care of storing the configuration of the models and handles methods
    ...
    Class attributes (overridden by derived classes):
        ...
        - **config_class** (:class:`~transformerslora.PretrainedConfig`) -- A subclass of
          :class:`~transformerslora.PretrainedConfig` to use as configuration class for this model architecture.
        - **load_tf_weights** (:obj:`Callable`) -- A python `method` for loading a TensorFlow checkpoint in a PyTorch
          model, taking as arguments:

            - **model** (:class:`~transformerslora.PreTrainedModel`) -- An instance of the model on which to load the
              TensorFlow checkpoint.
            - **config** (:class:`~transformerslora.PreTrainedConfig`) -- An instance of the configuration associated to
              the model.
    ...
    def save_pretrained(
        ...
        `:func:`~transformerslora.PreTrainedModel.from_pretrained`` class method.
        ...
        Parameters:
            ...
            config (:obj:`Union[PretrainedConfig, str, os.PathLike]`, `optional`):
                Can be either:
                    ...
                    - an instance of a class derived from :class:`~transformerslora.PretrainedConfig`,
                    - a string or path valid as input to :func:`~transformerslora.PretrainedConfig.from_pretrained`.
                ...
                be automatically loaded when:
                    ...
                    - The model is a model provided by the library (loaded with the `model id` string of a pretrained
                      model).
                    - The model was saved using :func:`~transformerslora.PreTrainedModel.save_pretrained` and is reloaded
...
class SquadHeadOutput(ModelOutput):
    """
    Base class for outputs of question answering models using a :class:`~transformerslora.modeling_utils.SQuADHead`.
...
def prune_conv1d_layer(layer: Conv1D, index: torch.LongTensor, dim: int = 1) -> Conv1D:
    ...
    Args:
        layer (:class:`~transformerslora.modeling_utils.Conv1D`): The layer to prune.
        index (:obj:`torch.LongTensor`): The indices to keep in the layer.
        dim (:obj:`int`, `optional`, defaults to 1): The dimension on which to keep the indices.

    Returns:
        :class:`~transformerslora.modeling_utils.Conv1D`: The pruned layer as a new layer with :obj:`requires_grad=True`.
    """
    index = index.to(layer.weight.device)
    W = layer.weight.index_select(dim, index).clone().detach()
    if dim == 0:
...
def prune_layer(
    ...
    Returns:
        :obj:`torch.nn.Linear` or :class:`~transformerslora.modeling_utils.Conv1D`: The pruned layer as a new layer with
