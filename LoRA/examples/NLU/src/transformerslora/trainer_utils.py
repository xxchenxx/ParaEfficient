...
class BestRun(NamedTuple):
    """
    The best run found by an hyperparameter search (see :class:`~transformerslora.Trainer.hyperparameter_search`).
...
def default_compute_objective(metrics: Dict[str, float]) -> float:
    ...
    metrics are provided to the :class:`~transformerslora.Trainer`, the sum of all metrics otherwise.
...
class TrainerMemoryTracker:
    ...
    Note that this tracker doesn't account for memory allocations outside of :class:`~transformerslora.Trainer`'s
    ...
    This also means that if any other tool that is used along the :class:`~transformerslora.Trainer` calls
    ``torch.cuda.reset_peak_memory_stats``, the gpu peak memory stats could be invalid. And the
    :class:`~transformerslora.Trainer` will disrupt the normal behavior of any such tools that rely on calling
