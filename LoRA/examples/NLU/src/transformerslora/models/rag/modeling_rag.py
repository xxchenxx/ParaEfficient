...
RAG_START_DOCSTRING = r"""
    ...
    The question encoder can be any `autoencoding` model, preferably :class:`~transformerslora.DPRQuestionEncoder`, and the
    generator can be any `seq2seq` model, preferably :class:`~transformerslora.BartForConditionalGeneration`.
    ...
    the ``generator``. It has been tested with :class:`~transformerslora.DPRQuestionEncoder` as the ``question_encoder``
    and :class:`~transformerslora.BartForConditionalGeneration` or :class:`~transformerslora.T5ForConditionalGeneration` as the
    ``generator``.
    ...
    Args:
        config (:class:`~transformerslora.RagConfig`):
            ...
            :meth:`~transformerslora.PreTrainedModel.from_pretrained` method to load the model weights.
        question_encoder (:class:`transformerslora.PreTrainedModel`):
            An encoder model compatible with the faiss index encapsulated by the ``retriever``.
        generator (:class:`transformerslora.PreTrainedModel`):
...
RAG_FORWARD_INPUTS_DOCSTRING = r"""
    Args:
        ...
        encoder_outputs (:obj:`tuple(tuple(torch.FloatTensor)`, `optional`)
            ...
            Used by the (:class:`~transformerslora.RagModel`) model during decoding.
